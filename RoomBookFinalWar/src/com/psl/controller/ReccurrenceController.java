...
//		TrainingDetailsDao tdao=new TrainingDetailsDao();
		...
		//	String day=request.getParameter("day");
			...
			System.out.println("");
			System.out.println("stime:"+stime+"\t etime "+etime+"\t room "+room+"\t recur: "+recur+"\t sdate: " +sdate+"\t range: "+range + " TType" + trainingType);
			...
			try {
			 ...
			 //  System.out.println(truncatedDate1+"  "+truncatedDate2);
			  // long timeDifference = truncatedDate2.getTime()- truncatedDate1.getTime();
			   ...
			   System.out.println("cal: "+c.getTime().getDay()+"  "+c1.getTime());
		...
		/*	public TrainingDetails(long trainingID, long employeeID, long roomID,
					...
					String startTime, String endTime, String ownerName)
*/
if(range.equalsIgnoreCase("no end date")){
		if(recur.equalsIgnoreCase("weekly")){
			...
			System.out.println("cal: "+c.getTime()+"\t newcal: "+newcal.getTime());
			list.add(new TrainingDetails( 1,id,ID,c.getTime(),c.getTime(),trainingType,stime,etime,ownerName,trainingName));
			...
			/*for(int i=0;i<2;i++){
				...
				int n1=c.getTime().getDate();
				while(n1!=n2){
					c.add(Calendar.DATE,1);

				}
				
				System.out.println(c.getTime()+" : "+c.getTime().getDay()+"  : "+c.getTime().getDate());
			}*/
		}
				...
				if(recur.equalsIgnoreCase("weekly")){
					...
					System.out.println("cal: "+c.getTime()+"\t newcal: "+c1.getTime());
					list.add(new TrainingDetails( 1,id,ID,c.getTime(),c.getTime(),trainingType,stime,etime,ownerName,trainingName));
	...
	for(TrainingDetails details : list){
		insert=tdd.isRoomAvailable(details.getRoomID(),	details.getStartDate()	,details.getEndDate()	,details.getStartTime()	, details.getEndTime());
		if(insert==false)
		{flag1=1;
		break;
		}
	...
	for(TrainingDetails details : list){
		...
		try {
			 map1 = statusdao.getStatus(details.getRoomID(), sDate, eDate);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(map1.isEmpty()){
			//room is available for all days selected, insert training details in training table 
			...
			//added code of loukik
			 trainingid=trainingid+tdao.showTrainingid(sDate,details.getStartTime(),details.getRoomID());
			...
			while( !start.after(end)){
				...
				statusdao.insertStatus(details.getRoomID(), targetDay,details,trainingid);
			...
			for(Date d:target)
			 {
				System.out.println("d id " + d);
				 boolean bool = tdao.isRoomAvailable(details.getRoomID(),d,eDate,details.getStartTime(),details.getEndTime());
				 ...
				 {*/
					 ...
					 for(Date d:target)
					 {
							...
							{						
								...
								statusdao.updateStatus(details.getRoomID(), targetDay, details.getStartTime(), details.getEndTime(),trainingid);
								flag=0;
								break;
							}
					 }
	...
	if(flag1==1 && s.equals("Administrator") )
	{
		...
		RequestDispatcher rd2=request.getRequestDispatcher("recurrenceForm.jsp");             		        		 
